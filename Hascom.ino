...
void showClima()
{ // Zeige Temperatur und Luftfeuchte im Fahrzeug an
  lcd.setCursor(0, 0);
...
byte getButton()
{
  ...
  while (!digitalRead(A1) or !digitalRead(A3))
  {
    ...
    if ((!digitalRead(A1)) && ((millis() - timer) > 5000))
    {
      setDaylight();
      break;
    }
    if ((!digitalRead(A3)) && ((millis() - timer) > 5000))
    {
      setNightlight();
      break;
    }
