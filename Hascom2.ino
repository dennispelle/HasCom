...
byte wtag, gmonat, gtag, gstunde, gminute, ominute = 61, gsekunde, zeitzone = 1, gpsdn, gpstn;
; // Zeitzone für sommer/winterzeit usw
int gjahr;
...
byte getlength(int N)
{ // überprüfe die anzahl der Ziffern und wandle sie in Pixelweite
  if (N < -99)
    return 72;
  else if (N < -9)
    return 54;
...
void getTemp()
{ // Hole die Tempereatur und die Luftfeuchtigkeit und speichere sie in Var: temperature und humidity
  // schaue ob die letzte Messung der der Temperatursenosren wenigstens eine sekunde her ist, ansonsten überspringen
  if ((tempsensorstage == 1) & ((TempUhr + 750) < millis()))
  {
    T2 = sensors.getTempCByIndex(0);
    tempsensorstage = 2;
  }
  if ((tempsensorstage == 2) & ((TempUhr + 1500) < millis()))
  {
    T1 = sensors.getTempCByIndex(1);
    tempsensorstage = 3;
    sensors.requestTemperatures();
  }
  if ((tempsensorstage == 3) & ((TempUhr + 2250) < millis()))
  {
    temperature = mySensor.getTemperature();
    tempsensorstage = 4;
  }
  if ((tempsensorstage == 4) & ((TempUhr + 3000) < millis()))
  {
...
{ // Startbildschirm, Zeigt die Uhrzeit und das Datum Groß an, dazu Clima und Temperaturen an.

  // Block für die Uhrzeit und das Datum
  getGpsClock();
  if (!gpst)
    getbatclock(); // schau ob die GPSzeit stimmt, wenn nicht hol die Zeit aus der Uhr
  if (!gpsd)
    getbatday(); // nochmal das gleiche mit dem Tag
  if ((first) || (gminute != ominute))
  { // schau, ob das Bild aktualisiert werden muss
    ominute = gminute;
  ...
  tft.setTextSize(3);
  tft.print(analogRead(A3) / 1023.0 * 20.0, 1);
  tft.setTextColor(RED, BLACK);
  tft.print("V  ");
  first = 0;
}
...
void wochentag()
{ // gibt einen String auf dem TFT aus
  getbatday();
  switch (wtag)
  {
  case 0:
...
void getGpsClock()
{ // Hole die Zeit über das GPSmodul und korrigiere Sie
  ...
  gstunde = gstunde % 24;
  if (gstunde < (zeitzone + Sommerzeit))
  { // Zeitkorrektur
    gtag++;
    if (gtag == 32)
    {
  ...
  if (gpsd & gpst & savetime)
    savedate();
  if (gpsd & gpst & savetime)
    savedate();
}
void savedate()
{ // Speichere die aktuelle Zeit im Batteriemodul
  tm.Day = gtag;
  tm.Month = gmonat;
...
void getbatclock()
{ // Hole die Zeit aus dem Batteriemodul
  RTC.read(tm);
  gstunde = tm.Hour;
  gminute = tm.Minute;
  gsekunde = tm.Second;
...
void getbatday()
{ // Hole das datum aus dem Batteriemodul
  RTC.read(tm);
  wtag = tm.Wday;
  gtag = tm.Day;
  gmonat = tm.Month;
...
// Globale Variablen fpr GPSstatus
void gpsstatus()
{ // Funktionsbildschirm für GPS, zeigt Satelitenanzahl, Koordinaten, Geschwindigkeit usw an

  while (ss.available() > 0)
    if (gps.encode(ss.read()))
      ; // hole die daten vom GPS sensor
  ...
  if (satelitenanzahl != gps.satellites.value())
    printSatAnz();
  if (latitude != gps.location.lat())
    printLatitude();
  if (longitude != gps.location.lng())
    printLongitude();
  if (hoehenmeter != gps.altitude.meters())
    printHoehenmeter();

  if ((gpstag != gps.date.day()) || (gpsmonat != gps.date.month()) || (gpsjahr != gps.date.year()))
    printGpsDatum();
  if ((gpssekunde != gps.time.second()) || (gpsminute != gps.time.minute()) || (gpsstunde != gps.time.hour()))
    printGpsTime();

  if (gpsspeed != gps.speed.kmph())
    printGpsGeschwindigkeit();
  if (degree != gps.course.deg())
    printGpsRichtung();
}
void printSatAnz()
{
...
{ // Oszifunktion :)
  ...
  if (xac > 9)
    xac = 0;
  if (px == 1)
    tft.drawFastVLine(0, 1, 127, WHITE);
  ...
  xac++;                   // Arraycounter +1
  if (x2 > xmax)
    xmax = x2; // Verlgeich ob höchster je gemessener Wert überschritten wurde
  if (x2 < xmin)
    xmin = x2;                                       // Vergleich ob geringster je gemessener Wert Unterschritten wurde
  ...
  if (px > 239)
    px = 1; // Wenn Positionszähler größer als Bildschirm, von vorn beginnen
  if ((timer + 2000) < millis())
  {
    ...
    tft.print(" Boardspannung:");
    tft.print((xav[0] + xav[1] + xav[2] + xav[3] + xav[4] + xav[5] + xav[6] + xav[7] + xav[8] + xav[9]) / 64.0, 1);
    tft.print("V  "); // Gebe durchschnittsspannung an
    tft.setCursor(-10, 155);
    tft.print(" Spannungsmaxi:");
    tft.print(xmax / 6.4, 1);
    tft.print("V   ");
    tft.setCursor(-10, 175);
    tft.print(" Spannungsmini:");
    tft.print(xmin / 6.4, 1);
    tft.print("V   ");
    tft.setCursor(-10, 195);
    tft.print(" Spannungsdiff:");
    tft.print((xmax - xmin) / 6.4, 1);
    tft.print("V   ");
    xmax = 0;
    xmin = 255;
  }
...
{ // Gsensor Kalibrieren//
  ...
  gky /= 100;
  gkz /= 100;
  gkx = gkx * 0.0061 - 2.0625;
  gky = gky * 0.0061 - 2.0625;
  gkz = gkz * 0.0061 - 3.0625;
}
void tempstatus()
{ // Temperaturdiagramm
  byte counter;
  if (first)
  {
    ...
    {
      tft.drawFastHLine(0, 20 * counter + 1, 240, BLACK);
      if (counter == 7)
      {
        tft.drawFastHLine(0, 20 * counter + 2, 240, BLACK);
        tft.drawFastHLine(0, 20 * counter, 240, BLACK);
      }
    }
  ...
  if ((timesincerequest + 2500) < millis())
  {
    ...
    if (px == 1)
      tft.drawFastVLine(0, 1, 161, WHITE);
    else if (!(px % 40))
      tft.drawFastVLine(px, 1, 161, BLACK);
    else
      tft.drawFastVLine(px, 1, 161, WHITE); // Radieren für neue Messewerte
    ...
    for (counter = 1; counter < 8; counter++)
    {
      ...
      if (counter == 7)
      {
        tft.drawPixel(px, counter * 20 + 2, BLACK);
        tft.drawPixel(px, counter * 20, BLACK);
      }
    }
    ...
    tft.print(" AussenTemp:");
    tft.print((141 - T1));
    tft.print(char(0xF7));
    tft.print("C  "); // Gebe durchschnittsspannung an
    tft.setCursor(-10, 205);
    tft.setTextColor(RED, BLACK);
    tft.print(" MotorTemp :");
    tft.print((141 - T2));
...
void option()
{ // Optionsmenu um Helligkeiten Sommer und Winterzeit einzustellen
  byte omenu = 1;
  ...
  if (!digitalRead(0))
  {
    ...
    while (out)
    {
      ...
      if (omenu < 1)
        omenu = 4;
      if (change)
      { // Bildschirm nur aktualisieren, wenn sich was geändert hat.
        if (omenu == 1)
...
void setup(void)
{ // Setupfunktion, Initialisieren von Sensoren, Auslesen des Speichers usw.
  ss.begin(GPSBaud);
  sensors.begin();
  sensors.requestTemperatures();
  mySensor.read();
